...
-   What are the actual subjects? These subjects are very helpful.

-   What is points possible/earned and the difference from TOTAL points
    possible/earned?
...
-   Rows are typically cases/participants/units of data collected
...
```{r}
d |> 
  select(student_id, total_points_possible, total_points_earned, TimeSpent_hours)
```
...
-   Number of cases (rows) decreased from 438 to 203 when cut off was
    raised to 90%.
...
```{r}
d |> 
  select(student_id, TimeSpent_hours) |> 
...
```{r}
d |>
  select(student_id, TimeSpent_hours, uv) |>
  arrange(desc(uv))
...
```{r}
m2 <- lm(FinalGradeCEMS ~ TimeSpent + subject + uv, data = d)
summary(m2)
```
